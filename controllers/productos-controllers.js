import { productoServices } from "../servicios/productos-servicios.js";    /*1*/
...
const nuevoProducto = (name,price,imageUrl) =>{  
    ...
    <a href="">Ver Producto</a>
     ...
     card.innerHTML = contenido;      /*2 */
    card.classList.add("produto");      /*3 prod-holder */
    return card;                     /*4*/
}

const producto = document.querySelector("[datos-productos]"); /*5*/
const consolas = document.querySelector("[datos-consolas]");
const diversos = document.querySelector("[datos-diversos]")

const render = async () =>{
    try{
        const listaProductos = await productoServices.listaProductos(); /*6*/
